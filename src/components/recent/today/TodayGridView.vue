        ...
        <div v-if="fileType == 'folder'" :class="[length == 0 ? 'pb-60' : '']">
            <div class=" bg-white shadow" >
                <ul role="list" class="divide-y divide-gray-200" ref="scrollFolder" 
                    v-infinite-scroll="loadMoreToday.clickNextPageTodayFolder"
                    :infinite-scroll-immediate-check="false"
                    :infinite-scroll-disabled="disableScroll"
                    infinite-scroll-watch-disabled="disableScroll">
                    <li v-for="(folder, index) in files" :key="folder.id" >
                        <div class="block hover:bg-gray-50" :class="[files.length == index+1 ? 'border-b border-gray-200' : '']">
                            <div class="flex items-center px-4 sm:px-6">
                                        ...
                                        <div class="hidden md:block">
                                            <div>
                                                ...
                                                <p class="mt-2 flex items-center text-sm text-gray-500">
                                                    ...
                                                    <button v-else type="button" @click="changeFavoriteStatus(true, folder)"
                                ...
                                <div>
                                    <DropdownOption class="mx-auto float-right mr-2 mb-2" :data="folder" :type="'folder'" />
                                </div>
                            </div>
                        </div>
                    </li>
        ...
        <div v-else :class="[length == 0 ? 'pb-40' : '']">
            ...
            <div class=" bg-white shadow">
                <ul role="list" class="divide-y divide-gray-200"
                    v-infinite-scroll="loadMoreToday.clickNextPageTodayFile"
                    :infinite-scroll-immediate-check="false"
                    :infinite-scroll-disabled="disableScroll"
                    infinite-scroll-watch-disabled="disableScroll">
                            ...
                            <div class="flex items-center px-4 sm:px-6">
                                <div class="flex min-w-0 flex-1 items-center">
                                    <div class="flex-shrink-0">
                                        ...
                                        <img v-if="isPicture(folder.mimetype)" :src="folder.url ? folder.url : folder.url_path"
                                            :alt="folder.name" class="h-12 w-12 " />
                                        <component v-else :is="fileIcon(folder.mimetype)"
                                            class="mx-auto h-12 w-12 flex-shrink-0 text-indigo-300" aria-hidden="true" />
                                    </div>
                                    <div class="min-w-0 flex-1 px-4 md:grid md:grid-cols-2 md:gap-4">
                                        <div>
                                            <p class="truncate text-sm font-medium text-gray-600">{{ folder.name }}
