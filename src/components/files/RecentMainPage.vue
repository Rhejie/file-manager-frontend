<template>
    ...
    <div v-if="todayFolders.length > 0 || todayFiles.length > 0" class=" w-full">
        <div
            class="flex flex-col md:flex-row items-start md:items-center justify-between pb-4 mb-2 border-b border-gray-300">
            <div>
    
                <h4 class="text-2xl font-bold leading-tight text-gray-700 mt-5">Today</h4>
            </div>
        </div>
        <TodayGridView :filesLength="getTodayList()" :data="todayFolders" :type="'folder'" :loading="loadingToday"
            :next="nextPageTodayFolder" />
        <TodayGridView v-if="todayFiles.length > 0" :data="todayFiles" :type="'file'" :next="nextPageTodayFile"
            :filesLength="getLenthOfWeekData()" :loading="loadingTodayFile" />
    </div>
    <div v-if="weekFolders.length > 0 || weekFiles.length > 0" class=" w-full">
        ...
        <RecentWeekList :type="'folder'" :data="weekFolders" :loading="loadingWeekFolders" :filesLength="getRecentLength()"
            :next="nextPageWeekFolder" />
        <RecentWeekList :type="'file'" :data="weekFiles" :loading="loadingWeekFiles" :filesLength="getLenthOfMonthData()"
    ...
    <div v-if="monthFolders.length > 0 || monthFiles.length > 0" class=" w-full">
        ...
        <RecentMonthList :type="'folder'" :data="monthFolders" :loading="loadingMonthFolders" :filesLength="getMonthData()"
            :next="nextPageWeekFolder" />
        <RecentMonthList :type="'file'" :data="monthFiles" :loading="loadingMonthFiles" :filesLength="getLenthOfYearData()"
    ...
    <div v-if="yearFolders.length > 0 || yearFiles.length > 0" class=" w-full">
        ...
        <RecentYearList :type="'folder'" :data="yearFolders" :loading="loadingYearFolders" :filesLength="yearFiles.length"
            :next="nextPageWeekFolder" />
        <RecentYearList :type="'file'" :data="yearFiles" :loading="loadingYearFiles" :next="nextPageYearFile" />
    </div>
</template>
<script>
...
import RecentMonthList from "../recent/month/RecentMonthList.vue";
import RecentYearList from "../recent/year/RecentYearList.vue";
