        ...
        <main class="flex-1 overflow-y-auto h-screen">
            <div
                ...
                <div>
                    ...
                    <h4 class="text-2xl font-bold leading-tight text-gray-700">Favorited Files</h4>
                </div>
            </div>
            ...
            <div v-if="!loadingFolders && !loadingFiles">
                <FavoriteFolderGridView v-if="!isList && folders.length > 0" :loading="loadingFolders" :type="'folder'" :favorites="folders"
                    :next="nextPageFolder" />
                <FavoriteListView v-else-if="isList && folders.length > 0" :loading="loadingFolders" :type="'folder'" :favorites="folders" :next="nextPageFolder" />
                <FavoriteFolderGridView v-if="!isList && files.length > 0" :loading="loadingFiles" :type="'file'" :favorites="files"
                    :next="nextPageFile" />
                <FavoriteListView v-else-if="isList && files.length > 0" :loading="loadingFiles" :type="'file'" :favorites="files" :next="nextPageFile" />
            </div>
            <div v-else>
                <GridSkeleton/>
            </div>
...
export default defineComponent({
    ...
    setup() {
        ...
        onMounted(async () => {
            ...
            setTitle('Favorites');
            emitter.on('REMOVE_FAVORITE_IN_FAVORITES', ({ type, data }) => {
                if (type == 'folder') {
